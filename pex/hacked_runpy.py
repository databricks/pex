"""This is a modified version of runpy.py.

In this version, we add a function,
  run_module_with_sys_modules_override(mod_name),
which behaves like
  runpy.run_module(mod_name, init_globals=None, run_name='__main__', alter_sys=True)
except that, instead of updating both sys.modules[__name__] and sys.argv[0],
it updates only sys.modules[__name__] but leaves sys.argv[0] alone.

For background, see https://github.com/pantsbuild/pex/issues/210 and
...
The following is the original docstring of runpy:
...
def run_module_with_sys_modules_override(mod_name):
    run_name = '__main__'
    _, loader, code, fname = _get_module_details(mod_name)
    pkg_name = mod_name.rpartition('.')[0]
    with _TempModule(run_name) as temp_module:
        mod_globals = temp_module.module.__dict__
        return _run_code(code, mod_globals, None,
